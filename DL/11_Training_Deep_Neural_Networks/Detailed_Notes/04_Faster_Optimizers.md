...
### ✅ AdamW — The Modern Default (Loshchilov & Hutter, 2019)

Adam + L2 regularization has a subtle bug: the weight decay term is entangled with the adaptive learning rate scaling, so different parameters get different amounts of regularization. **AdamW** fixes this by **decoupling** weight decay from the gradient update.

$$\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \eta \hat{\mathbf{m}} \oslash \sqrt{\hat{\mathbf{s}} + \varepsilon} - \eta \lambda \boldsymbol{\theta}$$

The weight decay term $\eta \lambda \boldsymbol{\theta}$ is applied *separately* from the adaptive update.
...
> [!IMPORTANT]
> **AdamW is now the recommended default** in most modern deep learning frameworks (PyTorch, TensorFlow 2.13+, Keras 3). Most state-of-the-art models (transformers, vision models) use AdamW with a cosine learning rate schedule. When in doubt, start with AdamW instead of Adam.
