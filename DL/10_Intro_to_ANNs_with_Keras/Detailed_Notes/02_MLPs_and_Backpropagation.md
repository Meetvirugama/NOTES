...
> [!WARNING]
> **The Dying ReLU Problem:** While ReLU fixes vanishing gradients, it has its own failure mode. If a neuron's weights are pushed to produce negative pre-activation values (z < 0) for all training examples, its gradient is permanently **0** — the neuron "dies" and never recovers.
>
> **Solutions to Dying ReLU:**
> - **Leaky ReLU**: `max(0.01z, z)` — small negative slope (0.01) keeps the gradient alive.
> - **ELU** (Exponential Linear Unit): Smooth negative outputs. Often outperforms ReLU but slower.
> - **SELU** (Scaled ELU): Self-normalizing — keeps activations centered with zero mean and unit variance.
> - **Practical default**: Use **`He initialization + ReLU`** together. He initialization sets weights large enough that dead neurons are rare at the start.
...
**🔗 Previous Module →** [01_Biological_to_Artificial_Neurons.md](01_Biological_to_Artificial_Neurons.md)  
**🔗 Next Module →** [03_Regression_and_Classification_MLPs.md](03_Regression_and_Classification_MLPs.md)
