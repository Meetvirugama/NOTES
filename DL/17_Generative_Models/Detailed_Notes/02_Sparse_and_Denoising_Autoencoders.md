...
A **denoising autoencoder (DAE)** is trained to reconstruct the **clean** input $x$ from a **corrupted** version $\tilde{x}$:

$$\mathcal{L}_{DAE} = \|x - g(f(\tilde{x}))\|^2$$

The corruption process $\tilde{x} = \text{corrupt}(x)$ can be:
1. **Additive Gaussian Noise**: $\tilde{x} = x + \epsilon$, $\epsilon \sim \mathcal{N}(0, \sigma^2)$
...
denoising_encoder = keras.Sequential([
    keras.layers.Flatten(),
    keras.layers.GaussianNoise(0.2),           # Active during training only
    keras.layers.Dense(100, activation="selu"),
