...
## 🚀 5. Modern Architectures (Post-2019) {#modern}

> [!IMPORTANT]
> The book covers architectures up to ~2018. In interviews and practice, you'll also encounter these critical modern architectures.

### EfficientNet (Tan & Le, 2019)
*   **Key Idea: Compound Scaling.** Instead of arbitrarily scaling depth, width, or resolution independently, EfficientNet scales all three dimensions **simultaneously** using a fixed ratio.
*   The base model (B0) is found via Neural Architecture Search (NAS). B1-B7 scale up uniformly.
*   **Result:** EfficientNet-B7 achieved 84.3% top-1 accuracy on ImageNet with 66M parameters, vs. ResNet-50's 76% with 25M — dramatically better efficiency.
*   Use via Keras: `keras.applications.EfficientNetV2B0()`

### Vision Transformer (ViT, Dosovitskiy et al., 2020)
*   **Key Idea:** Apply the Transformer architecture (from NLP) to images by splitting each image into fixed-size **patches** (e.g., 16×16 pixels), flattening them, adding position embeddings, and feeding them as a sequence to a standard Transformer encoder.
*   No convolutions at all — pure self-attention.
*   **Catch:** Requires enormous amounts of data to outperform CNNs (pre-training on JFT-300M or ImageNet-21K). With limited data, CNNs still win.
*   Foundation of all modern vision-language models (CLIP, DALL-E, etc.)

### ConvNeXt (Liu et al., 2022)
*   **Key Idea:** "Modernize" a standard ResNet by incorporating design lessons from Vision Transformers (larger kernels, fewer normalization layers, inverted bottlenecks, GELU activation).
*   Result: Outperforms ViT on many benchmarks **without self-attention**, using pure convolutions.
*   Shows CNNs and Transformers are more similar than initially thought.

| Architecture | Params | ImageNet Top-1 | Key Innovation |
|---|---|---|---|
| ResNet-50 (2015) | 25M | 76.1% | Skip connections |
| EfficientNet-B7 (2019) | 66M | 84.3% | Compound scaling |
| ViT-L/16 (2020) | 307M | 87.1% | Image patches + Transformers |
| ConvNeXt-XL (2022) | 350M | 87.8% | Modernized ConvNet |
