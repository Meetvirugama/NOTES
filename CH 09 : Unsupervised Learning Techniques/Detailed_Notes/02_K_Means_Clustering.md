...
2.  **K-Means++:** Instead of placing the initial centroids completely randomly, the K-Means++ algorithm places the first one randomly, but mathematically forces the subsequent initial centroids to be placed as far away from each other as possible. This drastically reduces the chance of converging to a bad solution. *(Scikit-Learn uses K-Means++ by default).*
