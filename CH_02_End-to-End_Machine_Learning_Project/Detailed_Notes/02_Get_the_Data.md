...
> [!WARNING]
> **Python 3.12+ Security Note:** `tarfile.extractall()` without a `filter` parameter triggers a deprecation warning and is considered a security risk (path traversal attacks). In modern Python, use `housing_tgz.extractall(path=housing_path, filter='data')` to extract safely. For production code, always validate archive contents before extraction.
