...
> [!NOTE]
> **However**, this does not mean data always trumps algorithms. For complex tasks (vision, language), the combination of good architecture (deep learning) AND massive data is what produces state-of-the-art results. The key insight is: don't under-invest in data quality and quantity.
...
The tension between overfitting and underfitting is formally known as the **Bias-Variance Tradeoff**:

$$\text{Generalization Error} = \text{Bias}^2 + \text{Variance} + \text{Irreducible Error}$$

*   **Bias:** Error from overly simplistic assumptions (underfitting). A linear model has high bias when the true relationship is nonlinear.
*   **Variance:** Error from excessive sensitivity to small fluctuations in the training set (overfitting). A high-degree polynomial has high variance.
*   **Irreducible Error:** Noise inherent in the data itself — no model can eliminate this.

| Scenario | Bias | Variance | Model Complexity |
|---|---|---|---|
...
> [!TIP]
> The sweet spot is a model complex enough to capture the true patterns (low bias) but not so complex that it memorizes noise (low variance). Regularization helps you find this sweet spot.
...
**3. "Confusing correlation with causation in data mining"** ❌
> ML models find correlations in data, not causal relationships. If a model finds that ice cream sales predict drownings, it doesn't mean ice cream causes drowning — both are caused by hot weather. Always validate ML insights with domain expertise before drawing causal conclusions.
...
**Q4: Explain the Bias-Variance Tradeoff and its relationship to overfitting/underfitting.**
> **A:**
> The generalization error of a model decomposes into three components: Bias² + Variance + Irreducible Error.
> *   **High Bias (Underfitting):** The model makes strong, incorrect assumptions and fails to capture the true pattern. Example: fitting a linear model to quadratic data.
> *   **High Variance (Overfitting):** The model is too sensitive to the training data and memorizes noise. Small changes in training data cause wildly different models.
> *   The goal is to find the sweet spot where both bias and variance are low. Regularization controls this tradeoff by constraining model complexity.

**Q5: What is the difference between feature selection and feature extraction?**
> **A:**
> *   **Feature Selection:** Choosing the most useful features from the existing set and discarding the rest (e.g., keeping only "income" and "age" out of 50 features for a loan approval model).
> *   **Feature Extraction:** Combining existing features to create more informative ones (e.g., PCA reduces 100 features to 10 principal components that capture 95% of the variance).
> Both are part of **Feature Engineering**, which also includes creating entirely new features from external data sources.
