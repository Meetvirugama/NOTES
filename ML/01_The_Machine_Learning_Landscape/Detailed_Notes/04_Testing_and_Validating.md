...
> [!TIP]
> **Stratified K-Fold** is preferred for classification tasks: it ensures each fold has approximately the same proportion of each class as the full dataset. Without stratification, some folds might have too few (or zero) examples of a rare class.
...
# Stratified K-Fold (for classification — default when using cv=int)
...
**3. "Data leakage — fitting the scaler on the full dataset before splitting"** ❌
> If you standardize features using the mean/std of the ENTIRE dataset (including test data), information from the test set leaks into training. Always fit the scaler ONLY on the training set, then transform both training and test sets with those same parameters. This applies to all preprocessing steps, not just scaling.
