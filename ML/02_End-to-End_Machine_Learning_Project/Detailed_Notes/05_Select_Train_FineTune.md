...
**3. "Interpreting feature importance as causation"** ❌
> `feature_importances_` measures how much each feature reduces impurity in the tree-based model. A high importance score means the model *uses* that feature heavily, not that it *causes* the target. Example: `INLAND` has importance 0.165 — this doesn't mean being inland *causes* lower house prices; it merely means the model finds this feature useful for splitting the data.

> [!TIP]
> **Beyond Grid/Random Search:** Modern hyperparameter optimization tools like **Optuna** or **scikit-optimize** use **Bayesian optimization** — they build a probabilistic model of the objective function and intelligently choose the next hyperparameter combination to evaluate. This often finds better hyperparameters in fewer iterations than RandomizedSearchCV.
