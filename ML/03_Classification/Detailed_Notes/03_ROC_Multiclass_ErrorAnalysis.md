...
| **Fβ Score** | $(1+\beta^2) \cdot \frac{P \cdot R}{\beta^2 \cdot P + R}$ | Both P&R high (β=1 → F1) | Either low | β<1 favors precision, β>1 favors recall |
...
| **MCC** | $\frac{TP \cdot TN - FP \cdot FN}{\sqrt{(TP+FP)(TP+FN)(TN+FP)(TN+FN)}}$ | Perfect (+1) | Worst (−1), Random (0) | **Most reliable single metric for imbalanced data** |

> [!TIP]
> **Matthews Correlation Coefficient (MCC)** is considered by many ML practitioners to be the most reliable single-number metric for binary classification, especially on imbalanced datasets. Unlike F1, it uses all four cells of the confusion matrix (including TN). A high MCC means the classifier is good at both detecting positives AND avoiding false positives. Use `sklearn.metrics.matthews_corrcoef(y_true, y_pred)`.
