...
> [!TIP]
> **R² Score (Coefficient of Determination)** is a complementary metric: $R^2 = 1 - \frac{\text{SS}_{\text{res}}}{\text{SS}_{\text{tot}}}$. It ranges from 0 (model explains nothing) to 1 (perfect fit). A negative R² means the model is worse than just predicting the mean. Use `sklearn.metrics.r2_score(y_true, y_pred)` or `model.score(X, y)`.
...
**Q3: When would you choose the closed-form Normal Equation over Gradient Descent?**
> **A:**
> Choose the Normal Equation when: (1) the number of features is small (< ~10,000), (2) data fits in memory, and (3) you want an exact solution with no hyperparameter tuning (no learning rate, no convergence check). Choose Gradient Descent when: (1) features exceed ~10,000 (Normal Equation becomes $O(n^3)$), (2) data doesn't fit in memory (Mini-batch/SGD can handle streaming), or (3) you're training online (SGD updates incrementally). In practice, most ML engineers default to GD-based approaches for flexibility.
