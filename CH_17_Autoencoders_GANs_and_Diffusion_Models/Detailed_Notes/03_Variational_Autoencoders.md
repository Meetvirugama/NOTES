...
The network needs to backpropagate gradients through the sampling step `z ~ N(μ, σ²)`. But **sampling is a stochastic operation** — it has no gradient!
...
Now:
- `ε` is sampled externally (not a parameter → no gradient needed).
- `μ` and `σ` are deterministic functions of `x` → **gradients flow freely** through them.
- The sample `z` still comes from `N(μ, σ²)` — **mathematically equivalent**.
...
Measures how much the learned distribution `q_φ(z|x) = N(μ, σ²)` deviates from the **prior** `p(z) = N(0, I)`.
...
The KL term pushes the **encoder distribution toward the standard normal prior** `N(0, I)`:
...
- Ensures the entire latent space is covered → can sample `z ~ N(0,I)` and decode meaningful outputs.
