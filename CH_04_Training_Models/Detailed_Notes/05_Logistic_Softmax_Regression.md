...
**The Gradient (for GD updates):**
$$\frac{\partial}{\partial \theta_j} J(\theta) = \frac{1}{m} \sum_{i=1}^m \left(\sigma(\theta^T x^{(i)}) - y^{(i)}\right) x_j^{(i)}$$

> [!TIP]
> **Elegant property of the sigmoid:** $\sigma'(t) = \sigma(t) \cdot (1 - \sigma(t))$. This makes the gradient computation very efficient — you can compute the derivative from the sigmoid's own output without calling exp() again.
...
> [!WARNING]
> **Scikit-Learn's `LogisticRegression` applies $\ell_2$ regularization by default!** The hyperparameter `C` (inverse of regularization strength) defaults to 1.0. Higher C = less regularization, lower C = more regularization. This is the **inverse** of the `alpha` parameter used in Ridge/Lasso. Don't forget: `C = 1/α`.
